...
export default function EditPost({ params }) {
  ...
  const handleSubmit = async (overrideStatus) => {
    ...
    const newData = { ...db };
    // Remove from both posts and services to avoid duplicate keys when type changes
    newData.posts = db.posts.filter(p => p.slug !== targetForm.slug);
    newData.services = db.services.filter(s => s.slug !== targetForm.slug);

    // Extract type since it is only used for UI separation, and keep DB clean
    const { type, ...dbItem } = targetForm;
    const today = new Date().toISOString().split('T')[0];

    if (targetForm.type === 'news') {
      newData.posts.unshift({ 
        ...dbItem, 
        date: dbItem.date || today,
        updatedAt: today
      });
    } else {
      newData.services.unshift({
        ...dbItem,
        updatedAt: today
      });
    }
    ...
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div style={{ fontSize: '14px', color: '#666' }}>
          <Link href="/admin/posts" style={{ color: 'var(--primary-color)', fontWeight: '600' }}>Bài viết</Link> &gt; Chỉnh sửa
        </div>
        <Link href="/admin/posts" style={{ color: '#666', fontSize: '14px' }}>&larr; Quay lại</Link>
      </div>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <h2 style={{ margin: 0 }}>CHỈNH SỬA {formData.type.toUpperCase()}</h2>
      ...
      <form style={{ backgroundColor: 'white', padding: '30px', borderRadius: '8px', border: '1px solid #ddd', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
